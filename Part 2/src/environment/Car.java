...
public class Car {
	...
	public boolean isSafe(Case c){
		for (int i = 0; i < this.length; i++){
			if(c.equals(new Case(this.leftPosition.absc + i, this.leftPosition.ord))){
				return false;
			}
		} return true;
	}
	...
	public boolean OOB(){
		if(this.leftToRight){
			if(this.leftPosition.absc - this.length > this.game.width){
				return true;
			}
		} else {
			if(this.leftPosition.absc + this.length < 0){
				return true;
			}
		}
		return false;
